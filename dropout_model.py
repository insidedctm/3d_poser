...
class _3DINNDropout(object):
    ...
    def _3D_mesh_Interpretor(self, heatmaps, image, gender, f_gt, c_gt, resize_scale, is_train=False, reuse=False):
        ...
        # image: [batch, h, w, 3]
        # gender: [batch, 1]
        # Uses dropout rather than batch normalisation
        ...
        with tf.variable_scope("3D_mesh_Interpretor") as scope:
            ...
            fc1 = tf.nn.dropout(lrelu(conv2d(input_, f_dim, name='g_h1_conv')), keep_prob=keep_prob)
            fc1_1 = tf.nn.dropout(lrelu(conv2d(fc1, f_dim, 3, 3, 1, 1, name='g_h1_1_conv')), keep_prob=keep_prob)
            fc2 = tf.nn.dropout(lrelu(conv2d(fc1_1, f_dim * 2, name='g_h2_conv')), keep_prob=keep_prob)
            fc2_1 = tf.nn.dropout(lrelu(conv2d(fc2, f_dim * 2, 3, 3, 1, 1, name='g_h2_1_conv')), keep_prob=keep_prob)
            fc3 = tf.nn.dropout(lrelu(conv2d(fc2_1, f_dim * 4, name='g_h3_conv')), keep_prob=keep_prob)
            fc3_1 = tf.nn.dropout(lrelu(conv2d(fc3, f_dim * 4, 3, 3, 1, 1, name='g_h3_1_conv')), keep_prob=keep_prob)
            fc4 = tf.nn.dropout(lrelu(conv2d(fc3_1, f_dim * 8, name='g_h4_conv')), keep_prob=keep_prob)
            fc4_1 = tf.nn.dropout(lrelu(conv2d(fc4, f_dim * 8, 3, 3, 1, 1, name='g_h4_1_conv')), keep_prob=keep_prob)
            fc5 = tf.nn.dropout(lrelu(conv2d(fc4_1, f_dim * 16, name='g_h5_conv')), keep_prob=keep_prob)
            fc5_1 = tf.nn.dropout(lrelu(conv2d(fc5, f_dim * 16, 3, 3, 1, 1, name='g_h5_1_conv')), keep_prob=keep_prob)
            out = tf.reshape(fc5_1, [self.config.batch_size, -1])
            out = tf.concat([out, tf.expand_dims(tf.cast(gender, tf.float32), 1)], 1)
            fc6 = tf.nn.dropout(linear(out, 1024, scope="g_fc6"), keep_prob=keep_prob)
            num_hidden = 512
            fc6_1 = tf.slice(fc6, [0, 0], [-1, 512])
            fc6_2 = tf.concat([tf.slice(fc6, [0, 512], [-1, 512]), scale_factor, shift_factor], 1)

            params = tf.nn.dropout(linear(lrelu(fc6_1), num_hidden, scope="g_fc6_1"), keep_prob=keep_prob)
            #
            beta_params = tf.slice(params, [0, 0], [-1, 256], name="beta_params")
            beta_params = tf.nn.dropout(linear(lrelu(beta_params), 128, scope="g_fc6_beta") , keep_prob=keep_prob)
            ...
            size = (self.config.keypoints_num - 1) * 4
            pose_params = linear(lrelu(pose_params), size, scope="g_fc6_pose")
            ...
            params_RT = tf.nn.dropout(linear(lrelu(fc6_2), num_hidden, scope="g_fc6_2"), keep_prob=keep_prob)
            ...
            T3_params = tf.nn.dropout(tf.nn.relu(linear(lrelu(T3_params), 128, scope="T3_params_2")), keep_prob=keep_prob)
