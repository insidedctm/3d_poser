...
flags.DEFINE_float("keep_prob", 0.8, "dropout probability")
FLAGS = flags.FLAGS
...
def main(_):
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        if FLAGS.is_train:
            filenames = glob.glob('../tf_code/tfrecords2/train/*.tfrecords')
            print(filenames)
            ...
            test_filenames = glob.glob('../tf_code/tfrecords2/test/*.tfrecords')
            ...
            m.J_c_sr_v, m.idx_sr_v, m.pmesh_sr_v, m.v_gt_v, m.J_3d_orig_sr_v = \
                m.get_data(test_filenames, with_idx=True)
